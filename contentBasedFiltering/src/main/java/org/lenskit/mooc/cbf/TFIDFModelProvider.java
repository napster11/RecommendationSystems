    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                ...
                double freq = work.containsKey(tag)?work.get(tag):0.0;
                if(freq == 0.0) {
                    double tagfreq = docFreq.containsKey(tag)?docFreq.get(tag):0.0;
                    docFreq.put(tag,tagfreq+1);
                }
                work.put(tag, freq+1);
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            if(entry.getKey() == 2231) {
                logger.info("term vector for item is " + entry.getValue().size());
            }

            double sqSum = 0.0;

            for(Map.Entry<String,Double> tagVector : tv.entrySet()) {
                double tagDocFreq = docFreq.containsKey(tagVector.getKey())?docFreq.get(tagVector.getKey()):0.0;
                tagVector.setValue(tagDocFreq*tagVector.getValue());
                sqSum += tagVector.getValue()*tagVector.getValue();
            }

            for(Map.Entry<String,Double> tagVector : tv.entrySet()) {

                if(sqSum == 0.0) {
                    tagVector.setValue(0.0);
                }else {
                    tagVector.setValue((tagVector.getValue()) / (Math.sqrt(sqSum)));
                }
            }

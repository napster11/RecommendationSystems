...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        double userVecSqSum = 0.0;

        for(Map.Entry<String, Double> x : userVector.entrySet()) {
            userVecSqSum += x.getValue()*x.getValue();
        }
        ...
        for (Long item: items) {
            ...
            double sum = 0.0;
            double result = 0.0;
            for(Map.Entry<String,Double> tags: iv.entrySet()) {
                double userProfileTagScore = userVector.containsKey(tags.getKey())?userVector.get(tags.getKey()):0.0;
                score += userProfileTagScore*tags.getValue();
                sum += tags.getValue()*tags.getValue();
            }
            if(sum == 0.0 || userVecSqSum == 0.0) {
                result = 0.0;
            }else {
                result = (score) / (sqrt(sum) * sqrt(userVecSqSum));
            }
            results.add(Results.create(item, result));
